...
use daw_transport::{AudioBuffer, Track, PPQN};
...
pub fn render_timeline(tracks: &[Track], tempo: f64, sample_rate: u32, channels: u16) -> AudioBuffer {
    ...
    for frame_idx in 0..total_samples {
        ...
        for track in tracks {
            for clip in &track.clips {
                ...
                let clip_total_frames = clip.audio.samples.len() / clip_channels;
                let clip_length_ticks =
                    samples_to_ticks(clip_total_frames as f64, tempo, clip_sample_rate);
                ...
                if position >= clip_start_tick && position < clip_end_tick {
                    ...
                    let sample_offset = ticks_to_samples(tick_offset, tempo, clip_sample_rate);
